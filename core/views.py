...
class HomeView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if request.user.id_tipo_usuario.id == 3:
            ...
            if UsuarioAsistencia.objects.filter(usuario_id=alumno.id,asistencia__fecha=fecha).exists():
                fila.append(UsuarioAsistencia.objects.filter(usuario_id=alumno.id,asistencia__fecha=fecha).values('tipo_asistencia__puntaje')[0]['tipo_asistencia__puntaje'])
            else:
                fila.append('0')
            #Crear lista de puntajes de todas las actividades
            puntajeList= []
            for t in tipoActividades:
                if TipoActividad.objects.filter(pk=t.id, actividades_tipos__usuarioactividad__usuario_id=alumno.id,actividades_tipos__fecha=fecha).exists():
                    puntajes = TipoActividad.objects.filter(pk=t.id, actividades_tipos__usuarioactividad__usuario_id=alumno.id,actividades_tipos__fecha=fecha).values(
            ...
            if Evaluacion.objects.filter(fecha=fecha).exists():
                puntajeEv = UsuarioEvaluacion.objects.filter(usuario_id=alumno.id,evaluacion__fecha=fecha)
                if puntajeEv:
